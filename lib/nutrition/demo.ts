...
const cuttingUserOnboarding: Partial<OnboardingData> = {
  ...
  allergies: [],
  preferred_proteins: ['chicken', 'fish', 'egg_whites'],
  wake_time: '6_7am',
...
const monday = cuttingPlan.weekPlan.find(d => d.day === 'monday');
if (monday) {
...
const wednesday = bulkingPlan.weekPlan.find(d => d.day === 'wednesday');
if (wednesday) {
...
const earlyTrainerOnboarding: Partial<OnboardingData> = {
  ...
  dietary_preference: 'standard',
  allergies: [],
  preferred_proteins: ['chicken', 'greek_yogurt', 'fish'],
  wake_time: '5_6am',
  first_meal_delay: 'immediate',
...
const earlyMonday = earlyPlan.weekPlan.find(d => d.day === 'monday');
if (earlyMonday) {
