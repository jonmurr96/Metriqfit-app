...
create or replace function public.apply_workout_plan_preview(preview_plan_id uuid)
returns uuid as $$
declare
  v_preview public.user_workout_plans;
begin
  select *
  into v_preview
  ...
  if not found then
    raise exception 'Workout preview not found';
  end if;

  if v_preview.lifecycle_state <> 'preview' then
    raise exception 'Selected workout plan is not a preview';
  end if;
...
create or replace function public.discard_workout_plan_preview(preview_plan_id uuid)
returns boolean as $$
declare
  v_preview public.user_workout_plans;
begin
...
grant execute on function public.apply_workout_plan_preview(uuid) to authenticated;
grant execute on function public.apply_workout_plan_preview(uuid) to service_role;
grant execute on function public.discard_workout_plan_preview(uuid) to authenticated;
grant execute on function public.discard_workout_plan_preview(uuid) to service_role;
