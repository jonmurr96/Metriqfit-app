...
export function determineProduceDecision(input: ProduceDecisionInput): ProduceDecision {
  ...
  // Gate non-mandatory produce once the daily fiber target is already met
  if (input.dailyFiberSoFarG !== undefined && !isMandatoryVeggieSlot) {
    const remainingDailyBudget = DAILY_FIBER_TARGET - input.dailyFiberSoFarG;
    if (remainingDailyBudget <= 0) {
      return {
        ...
        preferredKinds,
        grams: 0,
        rationale: "Daily fiber target already met.",
        fiberTargetG,
        fiberGapG,
      };
    }
